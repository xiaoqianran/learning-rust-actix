...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      {
        title: "Rust + Actix 实战学习 · 系统路径",
      },
      {
        name: "description",
        content:
          "Rust + Actix-web 中文交互教程：系统路径、讲解+源码+模拟 Demo、Catppuccin 主题与 REST 工坊。",
      },
    ],
    links: [
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => {
    ...
    for (const tr of orderedTracks()) init[tr] = tr === contTrack || tr === "Rust基础";
    return init;
  });
